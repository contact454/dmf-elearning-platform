...
export interface StandardError {
  ...
  message: string;
  details?: Record<string, unknown>; // Safe details only (no PII)
}
 ...
 * Maps to HTTP 400.
 */
export function makeValidationError(details: string | Record<string, unknown>): StandardError {
  return {
 ...
 * Maps to HTTP 404.
 */
export function makeNotFound(entity: string, id?: string): StandardError {
  return {
    code: 'NOT_FOUND',
 ...
 * Maps to HTTP 403.
 */
export function makeForbidden(role?: string, permission?: string): StandardError {
  return {
    code: 'FORBIDDEN',
 ...
 * Maps to HTTP 409.
 */
export function makeConflict(reason: string): StandardError {
  return {
    code: 'CONFLICT',
 ...
 */
export function makeIdempotentReplay(reference: Record<string, string>): StandardError & { replayed: true } {
  return {
 ...
 * Maps to HTTP 503.
 */
export function makeTransientFailure(reason: string): StandardError {
  return {
 ...
 */
export function getHttpStatusCode(error: StandardError, defaultCreated: boolean = false): number {
