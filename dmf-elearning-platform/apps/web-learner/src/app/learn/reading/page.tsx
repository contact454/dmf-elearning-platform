...
export default function SmartLibraryPage() {
  ...
  const [recommended, setRecommended] = useState<ReadingContent[]>([]);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [contentData, recommendedData, statsData, levelsData] = await Promise.all([
          getReadingContent({ limit: 20 }),
          getRecommendedReading(TEMP_USER_ID, 5),
          getReadingStats(),
          getReadingLevels(),
        ]);
        ...
        toast.success('Library loaded!', `${contentData.items.length} articles available`);
      } catch (err) {
  ...
  const handleFilter = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getReadingContent({
