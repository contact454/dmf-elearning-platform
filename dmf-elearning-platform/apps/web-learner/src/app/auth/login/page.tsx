...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    setError(null);
    try {
      const response = await fetch('http://localhost:3002/api/system/user/login', {
        method: 'POST',
      ...
      if (!response.ok) {
        // Backend returns { error: { code, category, message } }
      ...
      // Save token to localStorage (simple MVP approach)
      if (result.token) {
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="backdrop-blur-md bg-white/60 rounded-2xl border border-white/20 shadow-2xl p-8 sm:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-semibold text-cyan-900 mb-2" style={{ fontFamily: 'Poppins, sans-serif' }}>
              Chào mừng trở lại
            </h1>
            <p className="text-cyan-700/80 text-sm" style={{ fontFamily: 'Open Sans, sans-serif' }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <label
                  htmlFor="rememberMe"
                  className="ml-2 text-sm text-cyan-800 cursor-pointer"
                  style={{ fontFamily: 'Open Sans, sans-serif' }}
                >
                  Ghi nhớ đăng nhập
                </label>
              ...
              <a
                ...
                className="text-sm text-cyan-700 hover:text-cyan-900 transition-colors duration-200"
                style={{ fontFamily: 'Open Sans, sans-serif' }}
              >
                Quên mật khẩu?
              </a>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-cyan-700" style={{ fontFamily: 'Open Sans, sans-serif' }}>
              Chưa có tài khoản?{' '}
              <a
                href="/auth/register"
        ...
        {/* Footer text */}
        <p className="mt-6 text-center text-xs text-cyan-700/60" style={{ fontFamily: 'Open Sans, sans-serif' }}>
          Bằng cách đăng nhập, bạn đồng ý với{' '}
          <a href="/terms" className="underline hover:text-cyan-900">
            Điều khoản dịch vụ
          </a>{' '}
          và{' '}
          <a href="/privacy" className="underline hover:text-cyan-900">
            Chính sách bảo mật
          </a>
        </p>
      </div>
