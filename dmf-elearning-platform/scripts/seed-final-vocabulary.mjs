#!/usr/bin/env node
/**
 * Seed Final Vocabulary to Database
 * Imports final-vocabulary.json into Vocabulary table
 */
...
const INPUT_FILE = path.join(__dirname, 'data/quality-expansion/final-vocabulary-v2.json');
...
const LEVEL_UNIT_MAP = {
  'A1': '60d11cbf-8f05-46c8-ad36-39b4ff465b67',
  'A2': 'fc72bb3f-44b0-4683-a31b-5617c7b9dd4a',
  'B1': '18a145f3-6d03-4349-9266-3234215748fb',
  'B2': '130201e8-cf1a-4fd3-9011-662023046f4b',
  'C1': '269ca24c-8fcf-48ba-ab2e-dd675ad2e4a1',
  'C2': '0bf8266c-d79d-45e0-9db4-e12a4cc43347',
};

async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║    🌱 SEED FINAL VOCABULARY TO DATABASE                    ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');

  // Load vocabulary
  const content = await fs.readFile(INPUT_FILE, 'utf-8');
  const vocabulary = JSON.parse(content);
  console.log(`📥 Loaded ${vocabulary.length} final words\n`);
  ...
  for (let i = 0; i < vocabulary.length; i += BATCH_SIZE) {
    ...
    for (const word of batch) {
      ...
      try {
        if (existingSet.has(word.word.toLowerCase())) {
          // Update existing word with examples if it doesn't have any
          await client.query(`
            UPDATE "Vocabulary"
            SET examples = $1, "meaningVi" = COALESCE(NULLIF("meaningVi", ''), $2), "updatedAt" = NOW()
            WHERE LOWER(word) = $3 AND (examples IS NULL OR examples = '[]')
          `, [
        ...
        if (!err.message.includes('duplicate')) {
          console.error(`   ⚠️ Error with ${word.word}: ${err.message}`);
        }
        skipped++;
      }
    }

    console.log(`[Batch ${batchNum}/${totalBatches}] Inserted: ${inserted}, Updated: ${updated}, Skipped: ${skipped}`);
  }
  ...
  console.log(`   Updated (added examples): ${updated}`);
