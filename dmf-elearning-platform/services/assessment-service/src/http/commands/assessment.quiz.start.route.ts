/**
 * HTTP route: POST /api/commands/assessment.quiz.start (Tuyến HTTP: Bắt đầu bài kiểm tra)
 */

import type { FastifyInstance } from 'fastify';
import { commandRegistry } from '@dmf/contracts';
import { handleAssessmentQuizStart } from '../../application/assessment.quiz.start.handler';
...
export function registerAssessmentQuizStartRoute(
  app: FastifyInstance,
  deps: {
    eventBus: EventBus;
    database: Database;
  ...
  app.post('/api/commands/assessment.quiz.start', async (request, reply) => {
    ...
    try {
      ...
      deps.logger.error('Assessment quiz start failed', error);
